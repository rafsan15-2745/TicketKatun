...
if (isset($_GET['reference'])) {
  ...
  if ($err) {
    ...
    header("Location: user.php?page=pay&error=payment");
    exit();
  }
  ...
  if ($response) {
    ...
    if (array_key_exists('data', $result) && array_key_exists('status', $result['data']) && ($result['data']['status'] === 'success') && ($result['data']['requested_amount'] === intval($amount))) {
      ...
      $ins = $conn->query("INSERT INTO payment (users_id, schedule_id, amount, ref, date) VALUES ('$user_id','$schedule_id', '$paid', '$reference', '$date')");
      ...
      if ($payment_id > 0) {
        ...
        header("Location: user.php?page=paid&now=true");
        exit();
      }
    } else {
      header("Location: user.php?page=pay&error=payment");
